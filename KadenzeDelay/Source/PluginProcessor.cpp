...
void KadenzeDelayAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    {
        mDelayTimeSmoothed = mDelayTimeSmoothed - 0.0001f * (mDelayTimeSmoothed = *mDelayTimeParameter);
...
float KadenzeDelayAudioProcessor::lin_interp(float sample_x0, float sample_x1, float inPhase)
{
